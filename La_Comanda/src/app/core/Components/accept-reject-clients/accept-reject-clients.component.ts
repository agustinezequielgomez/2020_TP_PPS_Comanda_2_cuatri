...
export class AcceptRejectClientsComponent implements OnInit {
  ...
  constructor(private dataBase: DatabaseService, private notification: NotificationService, public nav: NavController) { }

  async ngOnInit() {
    this.dataBase.getDataStream<DBUserDocument>(DataBaseCollections.users, x => x.where('user.enabled', '==', null))
  ...
  async resolveClient(client: Client, accepted: boolean) {
    try {
      ...
      await this.notification.sendEmail(client.email, (accepted) ? 'Bienvenido a La Comanda' : 'Usuario rechazado', !accepted);
    } catch (err) {
      console.log(err);
    }
  }
